function addToMasterVtxList(vtxList, idxList, newVertices, newIndices)
{
    ...
    for(let i=initIdxSize; i< idxList.length; i++) //from newest index and on...
    {
        ...
        if(newIndices[i-initIdxSize] == 0) idxList[i] = 0; //if idx is 0, use raw "0th" index for changing colours
    }
}

function editVerticesInVtxList(vtxList, offset, newVertices)
{
//todo?
}
...
function resizeCanvas()
{
    ...
    canvas.width = Math.round(document.body.clientWidth* dPR);//width of window
    canvas.height = Math.round(document.body.clientHeight* dPR);
}
